...
class SearchRepository {
  ...
  Future<SummeryModel> getSampleListData(startDate, endDate, statusId, page,
      serviceId, invoiceNum, sampleId) async {
    print("------------------> date <----------------------");
    print(statusId);
    print(startDate);
    print(endDate);
  ...
  Future<TableRowDesignModel> getTableRowDesignItem(
      serviceId, itemId, groupItemId, patientServiceIds) async {
    print("repo item Id ${itemId}");
    print("repo service Id ${serviceId}");
    if (patientServiceIds == null) {
      dynamic response = await _apiServices.getApiData(
      ...
      dynamic response = await _apiServices.getApiData(
          '/Item/GetTableRowDesignByItemId?itemId=$itemId&machineId=&groupItemId=$groupItemId&serviceId=$serviceId,${patientServiceIds ?? ""}');
      return TableRowDesignModel.fromJson(response);
    }
  }
