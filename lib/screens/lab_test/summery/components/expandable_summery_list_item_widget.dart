...
class _ExpandableSummeryListItemState extends State<ExpandableSummeryListItem> {
  ...
  bool showMarkAsDelivered = false;
  int count = 0;
  ...
  void initState() {
    ...
    summeryVM.showMarkAsDelivered(patientServices: widget.patientServiceList??[]);
  ...
  Widget build(BuildContext context) {
    print("item data ${widget.summeryListItem}");
    print("================");
    print((summeryVM.markAsDelivered.value));

    // print("data${  tableRowDesignVm.tableRowDesignListItem.value.design![0].unit?.name}");
    return Column(
      children: [
        if (summeryVM.markAsDelivered.value == true)
          TextButton(onPressed: ()async {
           await _globalMarkasDeleveredAlertDialog( summeryListItem:
...
//////////////////change 0 to null
  ...
  _approvedAlertDialog({required PatientServices patientServices}) async {
    print("patient service data ${patientServices.item}");
  ...
  _globalMarkasDeleveredAlertDialog(
      {required Item summeryListItem, }) async {
    await showDialog(
        context: context,
        builder: (context) {
          return StatefulBuilder(
                ...
                InkWell(
                    onTap: () {
                      setState(() {
                        ...
                        summeryListItem.invoiceDate =
                            DateTimeConverter.dateOfTiemConterter(
                                summeryListItem.invoiceDate)
                                .toString();
                        summeryListItem.dueDate =
                            DateTimeConverter.dateOfTiemConterter(
                                summeryListItem.dueDate)
                                .toString();
                        for(int index =0; index< (summeryListItem.patientServices?.length??0); index++) {
...
*/
                          ...
                          summeryListItem.patientServices![index].labStatusId =
                          3;
                          print("widget ${summeryListItem}");
                        }
                        summeryListItem.labStatusId = 3;
                        dynamic data = {
  ...
  _markeAsPrintedAlertDialog(
    ...
    print("markes printed ${summeryListItem.patientServices?[index].itemId}");
    print("markes printed ${summeryListItem.patientServices?[index]}");
    await showDialog(
        context: context,
        builder: (context) {
          return StatefulBuilder(
            builder: (context, setStates) => SingleChildScrollView(
              child: AlertDialog(
                title: Row(
                  mainAxisAlignment: MainAxisAlignment.end,
                  children: [
                ...
                content: Container(
                  ...
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text("Are you print test report? "),
                      const SizedBox(
                        height: 10,
                      ),
                    ],
                ...
                actions: [
                  //  Text("data${summeryListItem.patientServices?[index]}"),
                  // Text("${summeryListItem.patientServices?[index].itemId}"),
                  InkWell(
                      onTap: () {
                        setState(() {
                          summeryListItem.labStatusId = 5;

                          print(
                              "===================== ${summeryListItem.patientServices?[index].itemId}");
                          summeryListItem.patientServices?[index].labStatusId =
                              5;
                          dynamic data = jsonDecode(jsonEncode(
                              [summeryListItem.patientServices?[index]]));
