  ...
  void initState() {
    tableRowDesignVm
      ...
      listController = List.generate(
          value.design?.length ?? 0, (index) => TextEditingController());
    });
          ...
          Expanded(
            child: Obx(() {
              switch (tableRowDesignVm.rxRequestStatus.value) {
                ...
                case Status.SUCCESS:
                  if (designList.length == null || designList.length == 0) {
          ...
          InkWell(
              onTap: () async {
                ...
                for (int i = 0; i < designList.length; i++) {
                  if (i == 0)
                    designList[i].patientServiceId = widget.patientService.id;
                  if (i == 1)
                    designList[i].patientServiceId =
                        widget.patientService.patientServiceIds;
                }
                print(
                    "design List Length /////////////////////???????<<<<M<M<M<M<M<M<M");
                print(designList.length);
