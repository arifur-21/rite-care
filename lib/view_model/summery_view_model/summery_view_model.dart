...
import '../../model/lab_test_model/summery_model0/patient_services/patient_services.dart' as ps;
...
class SummeryViewModel extends GetxController {
  ...
  RxBool isFilterClear = false.obs;
  ...
  RxBool clearBtn = false.obs;
  RxBool markAsDelivered = false.obs;



  showMarkAsDelivered({required List<ps.PatientServices> patientServices}){
    int count  = 0;
    for (ps.PatientServices pService in patientServices ?? []) {
      if (pService.labStatusId == 5) {
        count++;
        print("counter $count");
      }
    }
    if (count == patientServices?.length) {
      markAsDelivered.value = true;
    }else
      {
  ...
  Future<void> getSummeryListData({
    ...
    print(
        "----------------------------11111111111111-> status id 11111111111111111------------------------>");
